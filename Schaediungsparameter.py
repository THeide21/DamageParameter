...
def VectorNewFieldOutput(odb,instance,frame,Name,eIDs,Vec):
    ...
    FieldOut.addData(position=INTEGRATION_POINT,instance=instance,labels=eIDs,data=Vec)
    odb.save()
    odb.close()    
...
#    print(odb_name,E,k,S_yield,part_Name,interactiveFlag,setName)
    ...
    for eID in eIDS:
        ...
        SWT_vec.append(creatPlane(pos,R))
        print(SWT_vec)
        #FS.append((calculateFS(E,k,S_yield,S_max,E_FS_min,E_FS_max),))
        
    print('Counter Plotting')
    ScalarNewFieldOutput(odb,odb.rootAssembly.instances[part_Name.upper()+'-1'],frames[-1],'SWT',tuple(eIDS),tuple(SWT),tuple(SWT))
    VectorNewFieldOutput(odb,odb.rootAssembly.instances[part_Name.upper()+'-1'],frames[-1],'SWT',tuple(eIDS),tuple(SWT),tuple(SWT_vec))
    odb=OPENodb(odb_name,odb_name+'.odb',interactiveFlag)
